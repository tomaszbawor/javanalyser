...
public class ExplanationService {
    ...
    private String generateExplanationRecursive(AstNode currentNode, DependencyGraph graph, Set<String> visitedNodes, int depth) {
        ...
        for (String calledMethodKey : calledMethodKeys) {
            ...
            if (calledNode != null) {
                ...
                String summary = generateExplanationRecursive(calledNode, graph, new HashSet<>(visitedNodes), depth + 1); // Pass copy of visited set
        ...
        // Remove node from visited set for alternative paths (if necessary, depending on exact logic)
        // visitedNodes.remove(currentNodeKey); // Might be needed if exploring different branches

        log.debug("Generated explanation for {}: {}", currentNodeKey, explanation.substring(0, Math.min(explanation.length(), 100)) + "...");
        return explanation;
    }

    // **Crucial Implementation Needed:** Find methods called *within* this specific method body.
    // This requires parsing the method's source code and resolving call targets using imports and context.
    // The existing DependencyExtractor might provide a starting point, but likely needs refinement
    ...
    private List<String> findCalledMethodKeys(AstNode methodNode, DependencyGraph graph) {
        ...
        // Attempt to parse just the method's code snippet if possible, or the whole class
        // This is simplified - real resolution is complex
        try {
            // Find the class node containing this method
            String className = methodNode.getPackageName() + "." + methodNode.getName().substring(0, methodNode.getName().indexOf('(')); // Heuristic
            AstNode classNode = graph.getNodes().stream()
                    .filter(n -> ("class".equals(n.getType()) || "interface".equals(n.getType())) && (n.getPackageName()+"."+n.getName()).equals(methodNode.getPackageName()+"."+className))
                            ...
                            cu.findAll(MethodCallExpr.class).forEach(call -> {
                                ...
                                call.findAncestor(MethodDeclaration.class).ifPresent(callerDecl -> {
                                    ...
                                    if (callerDecl.getNameAsString().equals(methodNode.getName())) { // Simplified check
                                        ...
                                        // Need to resolve 'call.getScope()' using imports, local variables, class hierarchy etc.
                                        ...
                                        // If not resolved via direct deps, might be internal call or unresolved external
    ...
    private String buildExplanationPrompt(AstNode node, Map<String, String> calledFunctionSummaries) {
        ...
        if (node.getReturnType() != null) prompt.append("Return Type: ").append(node.getReturnType()).append("\n");
        ...
        prompt.append("\n--- Task ---\n");
        prompt.append("1. Provide a step-by-step explanation of the logic within the source code provided above (Method: ").append(node.getName()).append(").\n");
        prompt.append("2. Describe the overall purpose of this method.\n");
        prompt.append("3. Incorporate the summaries of the called functions (listed above) into your explanation to describe the end-to-end flow when this method is executed.\n");
        prompt.append("4. Produce a concise, combined summary paragraph of what this method achieves, considering the actions of the functions it calls.\n");
